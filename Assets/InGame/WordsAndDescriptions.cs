...
public class WordsAndDescriptions: MonoBehaviour
{
    ...
    public static Dictionary <string, string> wrongAnswersDict;
    ...
    public async void Start()
    {
        ...
        wrongAnswersDict = new Dictionary<string, string>();

        if (!File.Exists(Path.Combine(Application.persistentDataPath, "WrongAnswers.json")))
        {
            wrongAnswersDict = await JSON_Control.LoadAddressableJsonFile("WrongAnswers.json");
        }
        else
        {
            wrongAnswersDict = JSON_Control.LoadAndroidJsonFile("WrongAnswers");
        }
